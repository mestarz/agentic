...
export function useChat({
  ...
  const handleSend = async (overridePrompt?: string, forcedSessionId?: string) => {
    ...
    const sessionId =
      forcedSessionId || currentSession?.id || `session-${Math.random().toString(36).substring(7)}`;
    addLog(
      `>>> [Debug] 请求参数 - Agent Model: ${appConfigs.agentModelID}, Core Engine: ${appConfigs.coreModelID}`,
    );
